...
import { ASSET_SCOPES, isStudioView, parseWorkbenchRoute, selectProject, selectRound, selectTask, serializeWorkbenchRoute, updateWorkbenchRoute } from './workbench-route.mjs';
...
function DeliveryComposer({ project, selection, deliveryName, deliveryCreating, onDeliveryNameChange, onCreate, onOpenAssets }) {
  const ready = selection.state === 'ready';
  return <div className="delivery-compose"><div><p className="eyebrow">{project?.name || '需要先在会话中建立项目'}</p><h2>{ready ? selection.eligibleAssets.length + ' 张成果可创建草稿' : '交付前完成选片'}</h2><small>{deliverySelectionMessage(selection)}</small></div><div className="delivery-controls">{ready ? <><input aria-label="交付名称" value={deliveryName} onChange={(event) => onDeliveryNameChange(event.target.value)} placeholder="交付名称" /><button type="button" className="command-button" disabled={deliveryCreating} onClick={onCreate}><PackageCheck size={16} />{deliveryCreating ? '正在创建' : '创建草稿'}</button></> : <button type="button" className="outline-button" onClick={onOpenAssets}><Bookmark size={16} />前往项目资产</button>}</div></div>;
}
...
function AssetCard({ asset, selected, selectionBusy, shared, onToggleSelect, onReview, onTrash, onRestore, onPreview, onInspect, onDownload, onCopy, onSetShared }) {
  ...
  return <article className={'asset-card ' + (asset.deletedAt ? 'is-trashed ' : '') + (selected ? 'is-selected' : '')}>
    ...
    {menuOpen && <div className="asset-action-menu" role="menu">{asset.deletedAt ? <button type="button" role="menuitem" onClick={() => runMenuAction(() => onRestore(asset.id))}><RotateCcw size={15} /><span>恢复资产</span></button> : <><button type="button" role="menuitem" onClick={() => runMenuAction(() => onCopy(asset))}><Copy size={15} /><span>复制图片</span></button><button type="button" role="menuitem" onClick={() => runMenuAction(() => onSetShared(asset, !shared))}><Share2 size={15} /><span>{shared ? '取消跨项目共享' : '共享到跨项目素材'}</span></button><button type="button" role="menuitem" onClick={() => runMenuAction(() => onInspect(asset.id))}><GitFork size={15} /><span>查看来源与评审</span></button><div className="asset-action-group"><button type="button" role="menuitem" onClick={() => runMenuAction(() => onReview(asset.id, 'keep'))}><Check size={15} /><span>保留</span></button><button type="button" role="menuitem" onClick={() => runMenuAction(() => onReview(asset.id, 'review'))}><CircleAlert size={15} /><span>待复核</span></button><button type="button" role="menuitem" onClick={() => { setAnnotating(true); setMenuOpen(false); }}><MessageSquareText size={15} /><span>添加批注</span></button><button type="button" role="menuitem" onClick={() => runMenuAction(() => onReview(asset.id, 'derive'))}><Sparkles size={15} /><span>标记为衍生方向</span></button><button type="button" role="menuitem" onClick={() => runMenuAction(() => onReview(asset.id, 'reject'))}><X size={15} /><span>不采用</span></button></div><button type="button" className="danger" role="menuitem" onClick={() => runMenuAction(() => onTrash(asset.id))}><Trash2 size={15} /><span>移入回收站</span></button></>}</div>}
...
function App() {
  ...
  const refreshStudio = useCallback(async (signal) => {
    const [studioData, providerData, projectData, taskTypeData, styleKitData, brandKitData, sharedAssetData] = await Promise.all([
      api('/api/studio', { signal }), api('/api/provider/status', { signal }), api('/api/projects', { signal }), api('/api/task-types', { signal }), api('/api/style-kits', { signal }), api('/api/brand-kits', { signal }), api('/api/shared-assets', { signal })
    ]);
  ...
  const refreshContext = useCallback(async (knownProjects, signal) => {
    ...
    if (!selectedProject) {
      ...
      const path = assetScope === 'studio' && !['library', 'shared-assets'].includes(view) ? assetPathForRoute(route) : null;
  ...
  const hasIneligibleDeliveryAssets = deliverySelection.state === 'needs_review';
  ...
  const completeDelivery = async () => {
    if (deliverySelection.state !== 'ready' || !selectedProject) { setError(deliverySelectionMessage(deliverySelection)); return; }
    try {
      setDeliveryCreating(true);
      const name = deliveryName.trim() || '交付-' + new Date().toISOString().slice(0, 10);
      const created = await api('/api/deliveries', { method: 'POST', idempotencyKey: uniqueKey('delivery-draft'), body: { projectId: selectedProject.id, name, assetIds: selectedDeliveryAssets.map((asset) => asset.id) } });
      await api('/api/deliveries/' + encodeURIComponent(created.id) + '/ready', { method: 'POST', idempotencyKey: uniqueKey('delivery-ready'), body: {} });
      await api('/api/deliveries/' + encodeURIComponent(created.id) + '/export', { method: 'POST', idempotencyKey: uniqueKey('delivery-export'), body: {} });
      setDeliveryName('');
      setNotice('交付已完成，图片已生成实体文件，可直接下载或复制。');
      await refresh();
    } catch (nextError) { setError(nextError.message || '无法完成交付。'); } finally { setDeliveryCreating(false); }
  };
  ...
  const refreshPlanVersions = async () => {
    if (!selectedRound) { setPlanVersions(EMPTY); return; }
    try {
      setPlanVersionsLoading(true);
      const plans = await api('/api/rounds/' + encodeURIComponent(selectedRound.id) + '/plan-versions');
      setPlanVersions(plans.planVersions || EMPTY);
    } catch (nextError) { setError(nextError.message || '无法读取计划版本。'); } finally { setPlanVersionsLoading(false); }
  };
  useEffect(() => {
    if (view !== 'prompts') return;
    void refreshPlanVersions();
  }, [view, selectedRound?.id]);
  ...
  return <main className="studio-shell" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const file = Array.from(event.dataTransfer.files).find((item) => item.type.startsWith('image/')); if (file && canImport) void upload([file]); }} onPaste={(event) => { const file = [...event.clipboardData.files].find((item) => item.type.startsWith('image/')); if (file && canImport) { event.preventDefault(); void upload([file]); } }}>
    ...
    <section className="work-surface">
      ...
      {notice && <div className="notice-strip"><Check size={16} /><span>{notice}</span><IconButton label="关闭提示" onClick={() => setNotice('')}><X size={15} /></IconButton></div>}
      {providerDetails && <section className="provider-details"><div><p className="eyebrow">高级设置</p><h2>生成配置详情</h2></div><div className="provider-details-content"><span>{providerDetails.configured ? '当前配置可用' : '当前配置未就绪'}</span><span>配置文件：<code>{providerDetails.providerEnvPath}</code></span>{providerDetails.providerId && <span>服务：{providerDetails.providerId} · {providerDetails.model}</span>}<span>能力：{providerDetails.capabilities?.edit ? '支持编辑' : '仅生成'}{providerDetails.capabilities?.referenceImage ? ' · 支持参考图' : ''}</span><span>Worker 并发：{providerDetails.workerConcurrency || provider?.workerConcurrency || 2}</span></div><IconButton label="关闭生成配置详情" onClick={() => setProviderDetails(null)}><X size={16} /></IconButton></section>}
       ...
       {view === 'projects' ? <ProjectIndex projects={projects} onOpenProject={(projectId) => navigateRoute(selectProject(route, projectId))} /> : view === 'project-overview' && selectedProject ? <ProjectOverview project={selectedProject} tasks={tasks} selectedCount={selectedAssets.length} onOpenTasks={() => navigateRoute({ view: 'tasks', taskId: null, roundId: null, compareRoundIds: [], runId: null })} onOpenAssets={() => navigateRoute({ view: 'assets', assetScope: 'project', taskId: null, roundId: null, compareRoundIds: [], runId: null })} onOpenDeliveries={() => navigateRoute({ view: 'deliveries', taskId: null, roundId: null, compareRoundIds: [], runId: null })} onOpenTask={(taskId) => navigateRoute(selectTask(route, taskId))} /> : view === 'tasks' && selectedProject ? <ProjectTaskList project={selectedProject} tasks={tasks} onOpenTask={(taskId) => navigateRoute(selectTask(route, taskId))} /> : view === 'studio-overview' ? <section className="overview-stage">
      ...
      </section> : view === 'prompts' ? <PromptWorkspace round={selectedRound} planVersions={planVersions} loading={planVersionsLoading} onRefresh={() => void refreshPlanVersions()} /> : view === 'runs' ? <section className="run-stage">
      ...
      </section> : view === 'guide' ? <LearningCenter onDismiss={dismissGuide} onNavigate={(view) => navigateRoute({ view })} /> : view === 'library' ? <CreativeLibrary taskTypes={taskTypes} styleKits={styleKits} brandKits={brandKits} sharedAssets={sharedAssets} onOpenProjects={() => navigateRoute({ view: 'projects' })} onOpenSharedAssets={() => navigateRoute({ view: 'shared-assets' })} /> : view === 'shared-assets' ? <SharedAssets assets={sharedAssets} onDownload={downloadAsset} onCopy={copyAsset} onSetShared={setAssetShared} onOpenProjects={() => navigateRoute({ view: 'projects' })} /> : view === 'deliveries' ? <CreatorDelivery project={selectedProject} selection={deliverySelection} deliveryName={deliveryName} deliveryCreating={deliveryCreating} onDeliveryNameChange={setDeliveryName} onCreate={() => void completeDelivery()} onOpenAssets={() => navigateRoute({ view: 'assets', assetScope: 'project', taskId: null, roundId: null, compareRoundIds: [], runId: null })} selectedAssets={selectedAssets} deliveries={deliveries} assets={assets} deliveryBusyId={deliveryBusyId} onDeliveryAction={deliveryAction} onRemoveSelection={(asset) => toggleSelection(asset.id)} onDownload={downloadAsset} onCopy={copyAsset} onArchiveProject={downloadProjectArchive} onArchiveDelivery={downloadDeliveryArchive} batches={deliveryBatches} batchName={batchName} selectedDeliveryIds={selectedDeliveryIds} onBatchNameChange={setBatchName} onToggleDelivery={toggleBatchDelivery} onBatchAction={batchAction} /> : view === '__legacy_deliveries__' ? <section className="delivery-stage">
        <DeliveryComposer project={selectedProject} selection={deliverySelection} deliveryName={deliveryName} deliveryCreating={deliveryCreating} onDeliveryNameChange={setDeliveryName} onCreate={() => void completeDelivery()} onOpenAssets={() => navigateRoute({ view: 'assets', assetScope: 'project', taskId: null, roundId: null, compareRoundIds: [], runId: null })} />
        <div className="delivery-selection">{selectedAssets.length ? selectedAssets.map((asset, index) => <div className="selected-asset" key={asset.id}><img src={'/api/assets/' + encodeURIComponent(asset.id) + '/file'} alt="" /><span>{asset.display?.label || '已选素材'} · {asset.review?.decision === 'keep' ? '已保留' : '未满足准入'}</span><IconButton label="移出当前选片" onClick={() => toggleSelection(asset.id)}><X size={15} /></IconButton></div>) : <div className="empty-stage"><PackageCheck size={28} strokeWidth={1.2} /><p>在资产视图选择已保留的图片后，在这里创建可审阅的交付草稿。</p></div>}</div>
        {deliveries.length > 0 && <div className="delivery-history"><p className="eyebrow">交付草稿与导出记录</p>{deliveries.map((delivery) => <article key={delivery.id}><div><b>{delivery.name}</b><span>{delivery.items?.length || 0} 张冻结资产 · {delivery.items?.every((item) => item.review?.decision === 'keep') ? '保留评审已冻结' : '历史评审快照'}</span></div><StatusPill value={delivery.status} />{delivery.status === 'draft' ? <div className="delivery-row-actions">{deliverySelection.state === 'ready' ? <button type="button" className="outline-button" disabled={deliveryBusyId === delivery.id} onClick={() => void deliveryAction(delivery, 'update')}>更新为当前选片</button> : <button type="button" className="outline-button" onClick={() => navigateRoute({ view: 'assets', assetScope: 'project', taskId: null, roundId: null, compareRoundIds: [], runId: null })}>完成选片</button>}<button type="button" className="command-button" disabled={deliveryBusyId === delivery.id} onClick={() => void deliveryAction(delivery, 'ready')}>准备交付</button></div> : delivery.status === 'ready' ? <div className="delivery-row-actions"><button type="button" className="outline-button" disabled={deliveryBusyId === delivery.id} onClick={() => void deliveryAction(delivery, 'draft')}>退回草稿</button><button type="button" className="command-button" disabled={deliveryBusyId === delivery.id} onClick={() => void deliveryAction(delivery, 'export')}>导出文件</button></div> : <span>已导出，内容以冻结快照为准</span>}</article>)}</div>}
        ...
        {view === 'assets' && <div className="asset-scope-control" aria-label="资产范围"><span>查看范围</span>{ASSET_SCOPES.filter((scope) => scope !== 'studio').map((scope) => <button type="button" key={scope} className={assetScope === scope ? 'is-active' : ''} disabled={(scope === 'round' && !selectedRound) || (scope === 'task' && !selectedTask) || (scope === 'project' && !selectedProject)} onClick={() => navigateRoute({ assetScope: scope })}>{ASSET_SCOPE_LABELS[scope]}</button>)}</div>}
        <div className="asset-stage-head"><div><span className="asset-count">{visibleAssets.length.toString().padStart(2, '0')}</span><span className="asset-count-label">{view === 'trash' ? '已移入回收站' : '张可用资产'}</span></div><div className="asset-stage-tools"><div className="asset-filter" aria-label="素材筛选"><SlidersHorizontal size={14} />{[['all', '全部'], ['generated', '生成'], ['import', '导入']].map(([value, label]) => <button type="button" key={value} className={assetFilter === value ? 'is-active' : ''} onClick={() => setAssetFilter(value)}>{label}</button>)}</div>{selectedAssets.length === 2 && <IconButton label="对比两张已选素材" onClick={() => { setPreviewZoom(1); setPreviewAssets(selectedAssets); }}><Eye size={16} /></IconButton>}<div className="asset-hint">{view === 'trash' ? '当前项目回收站' : selectedAssetIds.size ? selectedAssetIds.size + ' 张已选择' : ASSET_SCOPE_LABELS[assetScope] + '资产'}</div></div></div>
        {view === 'assets' && selectedProject && <AssetSelectionStrip assets={selectedAssets} onRemove={toggleSelection} onClear={() => void clearSelection()} onDownloadArchive={() => downloadProjectArchive(selectedAssets.map((asset) => asset.id))} onPreview={(nextAssets) => { setPreviewZoom(1); setPreviewAssets(nextAssets); }} />}
        {visibleAssets.length ? <div className="asset-grid">{visibleAssets.map((asset) => <AssetCard key={asset.id} asset={asset} selected={selectedAssetIds.has(asset.id)} selectionBusy={selectionBusyIds.has(asset.id)} shared={sharedAssetIds.has(asset.id)} onToggleSelect={markAsDeliverable} onReview={review} onTrash={trash} onRestore={restore} onInspect={inspectAsset} onDownload={downloadAsset} onCopy={copyAsset} onSetShared={setAssetShared} onPreview={(nextAssets) => { setPreviewZoom(1); setPreviewAssets(nextAssets); }} />)}</div> : <div className="empty-stage asset-empty">{view === 'trash' ? <Archive size={30} strokeWidth={1.15} /> : <Inbox size={30} strokeWidth={1.15} />}<p>{view === 'trash' ? '当前项目回收站为空' : (assetScope === 'round' && !selectedRound ? '请先从任务上下文选择轮次，再查看本轮结果。' : '当前范围内暂未找到资产。')}</p>{view === 'assets' && <button type="button" className="outline-button" onClick={() => inputRef.current?.click()}><Upload size={16} />导入图片</button>}</div>}
      </section>}
    </section>
