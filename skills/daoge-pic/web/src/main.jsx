  ...
  return <main className="studio-shell" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const file = Array.from(event.dataTransfer.files).find((item) => item.type.startsWith('image/')); if (file) void upload([file]); }} onPaste={(event) => { const file = [...event.clipboardData.files].find((item) => item.type.startsWith('image/')); if (file) { event.preventDefault(); void upload([file]); } }}>
    <aside className="studio-rail">
      ...
      <div className="context-detail">
        ...
        {tasks.map((task) => <button type="button" key={task.id} onClick={() => { setActiveTaskId(task.id); setActiveRoundId(null); }} className={'context-item ' + (selectedTask?.id === task.id ? 'is-active' : '')}><span>{task.name}</span><StatusPill presentation={task.id === selectedTask?.id ? selectedTaskStatus : taskPresentation(task)} /></button>)}
        {selectedTask && <div className="round-list">{rounds.map((round) => <button type="button" key={round.id} onClick={() => setActiveRoundId(round.id)} className={'round-item ' + (selectedRound?.id === round.id ? 'is-active' : '')}><span>{({ exploration: '探索', refinement: '优化', variation: '变体', edit: '编辑', fill: '补图' })[round.purpose]}</span><StatusPill value={round.status} scope="round" /></button>)}</div>}
      </div>
