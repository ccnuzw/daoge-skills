...
test('5.9.0 is the stable GitHub Release contract while 5.8.0 remains immutable history', () => {
  const packageJson = JSON.parse(read('package.json'));
  const packageLock = JSON.parse(read('package-lock.json'));
  const currentDocs = `${skill}\n${readme}\n${spec}`;

  assert.equal(packageJson.version, '5.9.0');
  assert.equal(packageLock.version, '5.9.0');
  assert.equal(packageLock.packages[''].version, '5.9.0');
  for (const document of [skill, readme, evidence]) {
    assert.match(document, /5\.9\.0/);
  }
  assert.match(`${skill}\n${readme}`, /稳定正式版本[^。\n]{0,120}5\.9\.0/);
  assert.doesNotMatch(`${skill}\n${readme}`, /5\.9\.0[^。\n]{0,80}尚未发布/);
  assert.doesNotMatch(`${skill}\n${readme}`, /稳定正式发布(?:仍)?(?:为|是)[^。\n]*5\.8\.0/);
  assert.match(readme, /daoge-pic-v5\.9\.0\/daoge-pic-5\.9\.0\.tgz/);
  ...
  const historicalEvidence = markdownSection(evidence, '## 5. daoge-pic 5.8.0 发布验证证据');
  assert.match(historicalEvidence, /daoge-pic-v5\.8\.0[\s\S]*GitHub Release[^\n]*\.tgz/);
  const releaseEvidence = markdownSection(evidence, '## 6. daoge-pic 5.9.0 发布验证证据');
  assert.match(releaseEvidence, /daoge-pic-v5\.9\.0[\s\S]*GitHub Release[\s\S]*500 次运行对比[\s\S]*1000 张选片[\s\S]*Chromium smoke/);
  assert.match(releaseEvidence, /0 vulnerabilities[\s\S]*未调用真实图片 Provider/);
  assert.doesNotMatch(releaseEvidence, /5\.9\.0 仍为尚未发布候选|不宣称 GitHub Release/);
});
