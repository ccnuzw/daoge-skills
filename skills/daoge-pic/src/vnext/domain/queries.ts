...
import { existsInStudioSql, joinInStudioSql, ScopedEntityType } from './studio-scope';
...
function project(row: StoredProject): Project { return { id: row.id, studioId: row.studio_id, name: row.name, description: row.description, templateId: row.template_id, templateVersion: row.template_version, status: row.status, version: row.version }; }
...
export function listProjects(db: StudioDatabase, studioId: string): Project[] { return (db.prepare('SELECT id, studio_id, name, description, template_id, template_version, status, version FROM projects WHERE studio_id = ? ORDER BY updated_at DESC').all(studioId) as unknown as StoredProject[]).map(project); }
...
export function searchStudio(db: StudioDatabase, studioId: string, query: string, limit = 25): StudioSearchResult[] {
  ...
  // ⚠️ 归属链一律走 studio-scope 的 `joinInStudioSql`：手写链的风险不是「不好看」，
  // 而是每一处都要靠人记得补 `studio_id` 谓词——这里原先的四个相关子查询就都没补。
  const runChain = joinInStudioSql('run_item', '');
  const runChainWhere = "WHERE (item.asset_id = asset.id OR item.id IN (SELECT relation.target_id FROM asset_relations relation WHERE relation.asset_id = asset.id AND relation.relation_type = 'output_of' AND relation.target_type = 'run_item')) AND project.studio_id = ? LIMIT 1";
  const rows = db.prepare(
    ...
    ' UNION ALL SELECT candidate.ordering, \'round\' AS entity_type, round.id AS entity_id, task.name AS label, task.project_id, round.task_id, round.id AS round_id, round.purpose, round.status ' + joinInStudioSql('creative_round', 'JOIN candidates candidate ON candidate.entity_type = \'round\' AND round.id = candidate.entity_id') + ' WHERE project.studio_id = ?' +
    ...
    '(SELECT task.project_id ' + runChain + ' ' + runChainWhere + '), ' +
    ...
    '(SELECT task.id ' + runChain + ' ' + runChainWhere + ') AS task_id, ' +
    '(SELECT round.id ' + runChain + ' ' + runChainWhere + ') AS round_id, ' +
    '(SELECT round.purpose ' + runChain + ' ' + runChainWhere + ') AS purpose, ' +
  ...
  ).all(studioId, safeQuery, boundedLimit, studioId, studioId, studioId, studioId, studioId, studioId, studioId, studioId, studioId) as Array<{ entity_type: StudioSearchResult['entityType']; entity_id: string; label: string; project_id: string; task_id: string | null; round_id: string | null; purpose: string | null; status: string | null }>;
