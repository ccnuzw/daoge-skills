...
文档类别：vNext 长期权威产品与架构规格。当前稳定正式版本为 [`5.13.0`](https://github.com/ccnuzw/daoge-skills/releases/tag/daoge-pic-v5.13.0)；`5.12.0` 及更早版本为不可变历史发布。实现状态与机器验证不由本文件重复声明，分别见 `src/vnext/`、`web/` 和 `docs/vnext_verification_evidence_zh.md`。
...
本规格定义 vNext 稳定产品与目标架构；稳定发布源码版本为 `5.13.0`，`5.12.0` 及更早版本为历史发布，具体实现状态、机器验证与发布证据由对应版本源码和独立验证记录证明，不从需求文字反推。后续代码、目录、命令或测试变化也不得自行改写本规格事实。
