...
| 就地建项目（宪法 §4.3 第四刀） | ✅ **收口 @ 2026-09-20（刀哥裁定）：不做**——画布是「当前项目内」的视图，项目新建入口由项目首页空态与工具条承载；宪法 §4.3 已补收口说明 |
| 画布级菜单项移入工具栏（批 1 待办 3） | ✅ **收口 @ 2026-09-20（刀哥裁定）：不做**——复制参考信息 / 导出 / 快捷键保留在右键菜单，作为不动用户习惯的已接受例外；宪法 §4.4 已补收口说明 |
| 9.9 出图过程「预计还要多久」 | ✅ **收口 @ 2026-09-20（刀哥裁定）：不做**——本版本不实现进度估计；宪法 §9.9 已从「未实施」改为「明确不做」 |

## 4. 实施日志

### 2026-09-20 · 创作手册重写（刀哥委托）

手册曾停在 5.13 的工程视角、顶部还挂一整块「离线策略」对照表。已按创作动线重写：
15 个主题 × 「界面上怎么做 / 回对话怎么说 / 记住这几条」，工程词退场 + 9 个必要术语首次出现即时解释；
离线策略整体退场（模型 / 死 CSS / 守卫同步清）；两条守卫按新标准重写。
全量 **916 / 914 通过 / 0 失败 / 2 跳过**，手册页真机复验通过。详见界面方案 §12。

**二稿(同日,刀哥反馈)**:「拿出去」太土——整本按产品口径重排:五步改「建立项目 / 提出需求 / 生成运行 / 选片评审 / 资产交付」,
主题标题换成产品陈述句(生成运行 / 生成历史 / 资产交付 / 回收站 / 安全边界),人话特征保留但口语俏皮退场;
新增「口语步骤名退场」守卫(拿出去 / 看出图 / 挑图 + 五步标签 + 页面骨架)。全量 **915 / 913 / 0 / 2**,真机复验零残留。
...
**委托延续**：刀哥指定排查「剩余界面路径（对话框 / 快捷键 / 画布）」。
**方法**：① **AX 树实测**（CDP `Accessibility.getFullAXTree`，不靠启发式）：9 个视图逐个拉全树，统计无名交互控件；
② 对话框电池：逐个打开 → 查 role / aria-modal / 初始焦点 / Tab 环 / Esc / 焦点归还 / 布局；
③ 快捷键对拍：`shortcut-model.mjs`（单一来源）逐条对实现的 keydown 分支，并在真机跑安全子集；
④ 画布交互电池：多选 / ⌘A / Esc / F / 框选 / 右键菜单 / 双击折叠 / 小地图 / 缩放钳位 / 文本视图。

**实测结论（正例）**：
- **AX 树零无名控件**（9 视图，含画布 1173 节点 / 手册 1127 节点）；创建类对话框的下拉/文本域都靠 `<label>` 包裹拿到名字（**代码扫描报的「表单控件无名字」误报经 AX 树撤销**）。
- 对话框语义全对：新建项目 / 新建任务 / 新建批次 / 编辑计划 / 生成服务设置（`role=dialog`、`aria-modal=true`、焦点进入、Esc 关闭、焦点归还）。
- 画布：多选（Shift/Cmd 点选）✓、⌘A 全选 33 ✓、Esc 清空 ✓、F 适应选择（单选中实测 scale 0.16→1.72）✓、编辑模式框选中 4 个 ✓、右键菜单 7 项 + Esc ✓、双击批次展开/收起（33↔37 节点，收回到 33）✓、小地图仅编辑模式渲染 ✓、滚轮缩放钳位 `[0.05, 5]`（顶部实测命中 5）✓、文本视图 `aria-labelledby` ✓。

**本轮修复（3 处，S18–S20；S14–S16 编号归并行会话的任务列表 / rail 修复）**：

| # | 现象 | 严重度 | 状态 |
| --- | --- | --- | --- |
| S18 | **浏览模式下右键「查看快捷键」是死点击**：面板渲染被 `editing &&` 门控，点了没有可见反应 | P2 | ✅ 已修（面板门控交给 `shortcutsOpen`）+ 浏览模式复验 |
| S19 | 快捷键表没写「编辑模式」前提：框选 / 微调 / 分组三条只在编辑模式生效 | P2 | ✅ 已修（`shortcut-model.mjs` 三条补「编辑模式：」前缀；面板与手册同源，一处改两处生效） |
| S20 | 从右键菜单打开快捷键面板后 **Esc 关不掉**（焦点留在 body，keydown 到不了画布处理器）；面板本身也没有可访问名称 | P2 | ✅ 已修（面板挂载即接焦 + `aria-label="画布快捷键"`）+ 复验（Tab → 关闭按钮、Esc 关闭） |

**守卫与回归**：`keyboard-manual` / `canvas-chrome` / `lineage-density` 等 14/14 绿；全量回归待树收口后补跑（并行会话正在做 Disclosure 统一重构，工作区 mid-flight）。
