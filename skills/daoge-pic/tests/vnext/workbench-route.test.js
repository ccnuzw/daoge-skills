...
test('Workbench route rejects unknown view and scope without inventing an entity selection', async () => {
  ...
  assert.deepEqual(parseWorkbenchRoute('?view=unknown&scope=all'), { view: 'assets', projectId: null, taskId: null, roundId: null, compareRoundIds: [], runId: null, assetScope: 'round' });
});
...
test('parent selections clear dependent Workbench route IDs while explicit run selection remains scoped', async () => {
  ...
  assert.deepEqual(selectProject(base, 'project_z'), { ...base, projectId: 'project_z', taskId: null, roundId: null, compareRoundIds: [], runId: null });
  assert.deepEqual(selectTask(base, 'task_z'), { ...base, taskId: 'task_z', roundId: null, compareRoundIds: [], runId: null });
  assert.deepEqual(selectRound(base, 'round_z'), { ...base, roundId: 'round_z', compareRoundIds: ['round_z'], runId: null });
  assert.deepEqual(updateWorkbenchRoute(base, { runId: 'run_z' }), { ...base, runId: 'run_z' });
});

test('Workbench route preserves explicit multi-round Studio overview comparison without an implicit run', async () => {
  const { parseWorkbenchRoute, serializeWorkbenchRoute } = await import('../../web/src/workbench-route.mjs');
  const route = parseWorkbenchRoute('?view=studio-overview&project=project_a&task=task_b&round=round_1&round=round_2&round=round_1&scope=task');
  assert.deepEqual(route.compareRoundIds, ['round_1', 'round_2']);
  assert.equal(route.runId, null);
  assert.equal(serializeWorkbenchRoute(route), '?view=studio-overview&project=project_a&task=task_b&round=round_1&round=round_2&scope=task');
});
