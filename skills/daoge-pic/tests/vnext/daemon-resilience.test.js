...
test('daemon hot-loads active Provider changes without a controlled restart', async () => {
  ...
  try {
    ...
    const prepared = await requestJson(started, '/api/rounds/' + round.body.data.value.id + '/plan', { method: 'POST', idempotencyKey: 'hot-provider-prepare', body: { expectedVersion: round.body.data.value.version, plan: { operation: 'generate', itemCount: 1, prompt: 'hot loaded provider image' } } });
