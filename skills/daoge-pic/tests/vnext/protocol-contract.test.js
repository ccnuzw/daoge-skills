...
test('runtime exposes protocol version separately from artifact version and rejects incompatible writers', async () => {
  ...
  try {
    ...
    assert.deepEqual(studio.body.data.protocol, { name: 'daoge-pic-skill-protocol', version: '3.0.0', runtimeVersion: '6.0.0', supportedRange: '>=3.0.0 <4.0.0' });
    ...
    const compatible = await fetchStudio(started, '/api/projects', { method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': 'compatible-protocol', 'x-daoge-skill-protocol': 'daoge-pic-skill-protocol/3.0.5' }, body: JSON.stringify({ name: 'accepted-compatible' }) });
    ...
    assert.equal(declaration.runtimeCompatibility, '>=6.0.0 <7.0.0');
  } finally {
