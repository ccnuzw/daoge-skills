...
test('CLI rejects Node versions with incompatible Windows file identity or SQLite support', () => {
  const modulePath = path.join(skillRoot, 'dist', 'vnext', 'cli', 'daoge.js');
  const script = `Object.defineProperty(process.versions, 'node', { value: '22.16.0' }); process.argv = ['node', 'daoge', '--help']; const { main } = require(${JSON.stringify(modulePath)}); void main().catch((error) => { process.stderr.write(error.message); process.exitCode = 1; });`;
  const rejected = spawnSync(process.execPath, ['-e', script], { encoding: 'utf8' });
  assert.equal(rejected.status, 1);
  assert.match(rejected.stderr, /Node\.js 22\.17\.0/);
  ...
  return new Response(JSON.stringify({
    ...
    data: {
      ...
      protocol: {
        ...
        runtimeVersion: protocol.runtimeVersion || '5.14.1',
