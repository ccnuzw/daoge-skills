...
当前工具版本：`3.14.2`（基于上游 `daoge-docs-v3.13.4`，包含本地门禁、索引新鲜度、跨版本 E2E、ADR 开发授权、跨版本工作台组合层与全项目功能浏览目录）。
...
“复制 Goal 提示”生成的是**准备** Goal 的提示词。它要求编程智能体先检查 `prepare-goal` 的结果，在 `blocked` 时停止并报告来源，在 `ready` 时再读取唯一下一任务，并在实现前等待你的确认。它不会绕过门禁，也不会授权 AI 直接改代码。
