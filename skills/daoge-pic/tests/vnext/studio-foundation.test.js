...
const { attachStudio, initializeStudio, ensureAssetBucket, ensureDeliveriesDirectory, enforceSensitiveAccess, sameWorkspaceRoot } = require('../../dist/vnext/studio/workspace');
...
test('persists canvas layout separately from Studio business facts', () => {
  ...
  try {
    ...
    const task = createTaskDraft(db, { studioId: initialized.manifest.studioId, projectId: project.id, name: '主视觉', idempotencyKey: 'layout-task' }).value;
    const round = createRoundDraft(db, { studioId: initialized.manifest.studioId, taskId: task.id, purpose: 'exploration', idempotencyKey: 'layout-round' }).value;
    ...
    const saved = saveCanvasLayout(db, { studioId: initialized.manifest.studioId, projectId: project.id, viewport: { x: 12, y: -8, k: 1.25 }, settings: { filter: 'selected', apiKey: 'must-redact' }, nodes: [{ entityType: 'task', entityId: task.id, x: 10, y: 20, width: 240, height: 112, groupId: 'group_context' }, { entityType: 'round', entityId: round.id, x: 340, y: 160, width: 230, height: 116, groupId: 'group_context' }, { entityType: 'asset', entityId: 'asset_layout_project', x: 620, y: 160, width: 174, height: 208 }], groups: [{ id: 'group_context', title: '这一批', groupType: 'round', x: 60, y: 120, width: 540, height: 180, metadata: { collapsed: false } }], links: [{ id: 'link_reference', sourceType: 'asset', sourceId: 'asset_layout_project', targetType: 'round', targetId: round.id, linkType: 'reference', label: '参考自', metadata: { manual: true } }, { id: 'link_group_context', sourceType: 'group', sourceId: 'group_context', targetType: 'task', targetId: task.id, linkType: 'context', label: '分组说明', metadata: { manual: true } }] });
    assert.equal(saved.projectId, project.id);
    assert.equal(saved.id === null, false);
    ...
    assert.deepEqual(saved.groups.map((group) => [group.id, group.title, group.groupType]), [['group_context', '这一批', 'round']]);
    assert.deepEqual(Object.fromEntries(saved.links.map((link) => [link.id, [link.sourceType, link.linkType, link.targetType, link.label]])), { link_reference: ['asset', 'reference', 'round', '参考自'], link_group_context: ['group', 'context', 'task', '分组说明'] });
    assert.deepEqual(Object.fromEntries(saved.nodes.map((node) => [node.entityType, node.groupId || null])), { task: 'group_context', round: 'group_context', asset: null });
    // The canvas is one per project: saving again updates the same layout rather than adding a second.
    const resaved = saveCanvasLayout(db, { studioId: initialized.manifest.studioId, projectId: project.id, viewport: saved.viewport, settings: saved.settings, nodes: saved.nodes, groups: saved.groups, links: saved.links });
    assert.equal(resaved.id, saved.id);
    assert.equal(db.prepare('SELECT COUNT(*) AS total FROM canvas_layouts WHERE project_id = ?').get(project.id).total, 1);
    assert.throws(() => saveCanvasLayout(db, { studioId: initialized.manifest.studioId, projectId: project.id, viewport: {}, settings: {}, nodes: [{ entityType: 'plan', entityId: round.id, x: 0, y: 0, width: 120, height: 120 }], groups: [], links: [] }), /Unsupported canvas layout node type/);
    assert.throws(() => saveCanvasLayout(db, { studioId: initialized.manifest.studioId, projectId: project.id, viewport: {}, settings: {}, nodes: [], groups: [{ id: 'group_context', title: '这一批', x: 0, y: 0, width: 120, height: 120 }], links: [{ sourceType: 'group', sourceId: 'missing_group', targetType: 'task', targetId: task.id, linkType: 'context' }] }), /group endpoint/);
    assert.throws(() => saveCanvasLayout(db, { studioId: initialized.manifest.studioId, projectId: project.id, viewport: {}, settings: {}, nodes: [], groups: [{ title: '缺少稳定 ID', x: 0, y: 0, width: 120, height: 120 }], links: [] }), /Canvas group id/);
    assert.throws(() => saveCanvasLayout(db, { studioId: initialized.manifest.studioId, projectId: project.id, viewport: {}, settings: {}, nodes: [{ entityType: 'asset', entityId: 'asset_layout_private', x: 0, y: 0, width: 174, height: 208 }], groups: [], links: [] }), /not available to this project/);
    assert.throws(() => saveCanvasLayout(db, { studioId: initialized.manifest.studioId, projectId: project.id, viewport: {}, settings: {}, nodes: [], groups: [], links: [{ sourceType: 'asset', sourceId: 'asset_layout_private', targetType: 'round', targetId: round.id, linkType: 'reference' }] }), /not available to this project/);
    ...
    assert.deepEqual(JSON.parse(event.payload_json), { projectId: project.id, nodeCount: 3, groupCount: 1, linkCount: 2 });
  } finally {
    closeStudioDatabase(db);
    cleanup(workspaceRoot);
  }
...
test('persists confirmed creative context with idempotency and optimistic versions', () => {
  ...
  try {
    ...
    const context = db.prepare('SELECT agent_project_id, agent_task_id, agent_round_id FROM studio_sessions WHERE id = ?').get(session.id);
    assert.equal(context.agent_project_id, createdProject.value.id);
    assert.equal(context.agent_task_id, task.value.id);
    assert.equal(context.agent_round_id, round.value.id);
