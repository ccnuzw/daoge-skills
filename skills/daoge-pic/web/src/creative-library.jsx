  ...
  return <section className="creative-library">
    <header className="library-masthead">
      <div>
        ...
        <h2>规则资料</h2>
        <p>任务类型、风格包和品牌包放在这里。共享图片在单独标签里查看。</p>
      </div>
    ...
    <details className="library-boundary library-boundary-top">
      <summary><BookOpen size={16} />资料说明</summary>
      <div><Library size={18} /><span>项目图片不出现在这里。需要跨项目使用的图片，必须从项目资产明确共享。</span></div>
      <div><BookOpen size={18} /><span>资料库不自动改变已确认计划，也不会触发生成。</span></div>
    </details>

    <div className="library-toolbar">
      <div className="library-search">
        <Search size={16} />
        <input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="搜索创作资料库" placeholder="搜索任务类型、风格或品牌规则" />
        {query && <button type="button" className="icon-button" title="清除搜索" aria-label="清除搜索" onClick={() => setQuery('')}><X size={15} /></button>}
      </div>
      <div className="library-filters" role="group" aria-label="资料类型">
    ...
    <div className="library-workbench">
      <section className="library-resource-index" aria-label="创作资料">
        ...
        </button>) : <div className="library-empty"><BookOpen size={24} /><p>没有符合条件的资料。</p></div>}
      </section>
      ...
      </article> : <article className="library-resource-detail is-empty"><BookOpen size={26} /><h3>选择一项资料</h3><p>查看可复用的任务、风格或品牌规则。</p></article>}
    </div>
  </section>;
}
