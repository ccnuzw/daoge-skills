...
文档类别：vNext 长期权威产品与架构规格。当前稳定正式版本为 [`5.12.0`](https://github.com/ccnuzw/daoge-skills/releases/tag/daoge-pic-v5.12.0)；`5.11.0` 为不可变历史发布。实现状态与机器验证不由本文件重复声明，分别见 `src/vnext/`、`web/` 和 `docs/vnext_verification_evidence_zh.md`。
...
- 查看并直接创建当前 Studio 中的项目、任务和轮次，优先使用模板、选项、默认值、字段提示和可点击示例减少创作者输入；创建后直接进入当前上下文，不显示额外的创建完成提示或推荐下一步卡片；项目模板必须能影响后续新建任务表单的推荐顺序、默认任务名、目标数量、画幅、首轮目的、素材需求、必补信息、变化维度、精修目标和保持约束；无法枚举的自定义字段必须明确说明应填写的输入素材、限制条件、使用渠道和交付用途。
...
Workbench 不提供独立聊天入口。Studio 直接创建只建立结构化上下文或草稿计划，不触发确认挑战、预检、Generation Run 或 Provider 调用；项目意图深化、创作计划起草、生成确认、恢复不确定结果仍以智能体会话为主。Workbench 只提供视觉操作、精细管理和受控运行操作。
...
Studio 常驻导航固定提供项目入口、全局学习中心入口和生成/运行状态入口；创作资料库与共享素材仍是 Studio 全局视图，可从学习中心、资料页或共享素材页互相进入，不属于具体任务、轮次或运行。资料库仍是 Studio 级资料页，但从已打开项目中点击“资料”时必须保留当前项目壳，便于查看规则后回到项目；学习中心也可以保留当前项目壳作为返回锚点。资料库、共享素材与学习中心不得保留任务、轮次、运行上下文，也不得把资料自动绑定到计划。项目入口后才显示项目概览、任务、项目资产、创作谱系、交付和项目回收站；项目首页与任务功能必须提供名称搜索、生命周期筛选和分页，项目概览内的任务摘要也必须有同等搜索、筛选和有界分页，禁止项目或任务数量增长后无限向下延伸。项目资产提供“全部成果 / 已选”的视觉选片视图，以服务端范围查询分页，默认每页 24 张并允许 16/24/32/48/64/96，筛选后返回精确总数。“全选本页”只选择当前筛选条件和当前页中的资产，取消全选不得抹掉已有评审。项目资产导入必须允许文件选择器、拖放一次接收多张图片，逐张建立稳定资产并汇总部分失败。交付图片必须提供明确的全选/取消全选。即使手工构造 Workbench 深链，项目资产与创作谱系也不得保留或序列化 `scope=studio`：有轮次时归一到轮次范围，有任务时归一到任务范围，否则归一到项目范围；跨项目图片只能通过明确共享后的共享素材模块访问。
...
学习中心必须是可搜索、可按专题筛选的 Studio 全局创作手册，而非仅有流程概述的静态说明。它至少覆盖项目/任务/轮次、计划与提示词证据、预检与能力、运行与异常恢复、资产导入及范围、视觉选片与评审、参考与衍生、创作谱系、资料库、交付、项目回收和安全边界。每项都要区分 Workbench 中可查看或可操作的事实与必须回到会话确认的动作；只能链接项目列表和资料库等全局视图，可以保留当前项目壳作为返回锚点，但不得在全局学习页虚构任务级上下文、聊天、计划编辑、确认入队、Provider 调用或密钥配置。手册中的创作谱系说明必须明确：项目内点“资料”不关闭当前项目，谱系资料节点和软连线只形成计划上下文或人工标注，不会预检、运行或调用 Provider。
...
`5.12.0` 稳定正式版已按本规格收敛创作谱系、Generation History 分页、协议协商、项目资产 ZIP、Provider 安全边界、模板化创建与 Workbench 可访问性边界。后续版本不得重新引入旧新混合入口。任何发布仍必须先满足本规格验收项、相关高风险技术设计和对应版本的真实验证证据。
...
本规格定义 vNext 稳定产品与目标架构；稳定发布源码版本为 `5.12.0`，`5.11.0` 及更早版本为历史发布，具体实现状态、机器验证与发布证据由对应版本源码和独立验证记录证明，不从需求文字反推。后续代码、目录、命令或测试变化也不得自行改写本规格事实。
