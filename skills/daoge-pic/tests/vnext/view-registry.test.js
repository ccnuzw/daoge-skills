...
/**
 * G12 扩展（界面方案 §4 S3 / §5.4 / 批 A A2/A3 与 B5）。
 *
 * 宽度档（`layout`）是**页面的属性**，所以它跟「视图 → 宿主」一样进注册表：
 * 每个 view 声明一次，`PageFrame` 消费它——任何 view 不得自写宽度。
 */
test('every view declares a layout tier, and PageFrame consumes it', async () => {
