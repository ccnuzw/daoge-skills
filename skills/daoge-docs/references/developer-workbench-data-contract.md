...
当前功能选择器从顶层 `features[]` 显示稳定 `id + title`；工作台的“项目功能浏览”选择器从 `project_features[]` 显示 `version + id + title`，选择后进入对应时间线。项目版本组合层和时间线中的每个完整功能都可以复制稳定 ID 与 Goal 准备提示。Goal 提示必须显式携带 `--version <目标版本>`；提示词不是 Goal 清单，必须要求智能体运行 `prepare-goal` 后读取结果；`blocked` 时停止，`ready` 后只允许读取 `goal-resume-context`，不得由浏览器直接运行或伪造 Goal 状态。目标版本只改变 Goal 的解析上下文，不改变 `config.current_version`、工作台执行视角、当前 Gate、发布判断或当前派生浏览器数据。
