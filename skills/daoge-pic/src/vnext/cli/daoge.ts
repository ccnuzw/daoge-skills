...
const commandSchemas: Record<string, CommandSchema> = {
  ...
  'provider-delete': { method: 'POST', flags: { '--profile': { kind: 'text', required: true }, '--force': { kind: 'boolean' } }, pathname: (v) => '/api/providers/' + encoded(v, '--profile') + '/delete', body: (v) => ({ force: v['--force'] === true }) },
  ...
  'provider-models': { method: 'POST', flags: { '--profile': { kind: 'text', required: true } }, pathname: (v) => '/api/providers/' + encoded(v, '--profile') + '/models', body: () => ({}) },
