...
function capabilities(options = {}) {
  return {
    ...
    edit: true,
    maskEdit: true,
    responsesGenerate: supportsResponsesGenerateMode(options.generatePath),
    responsesReferenceEdit: supportsResponsesReferenceMode(options.editPath, 'reference-assisted'),
  };
}
...
async function generate(request = {}) {
  const req = {
    ...
    model: request.model,
    responsesModel: request.responsesModel,
  ...
  if (supportsResponsesGenerateMode(req.generatePath)) {
    return requestWithFallback({
      fileBase: request.id || 'image',
      label: 'responses prompt-only',
      primary: () => requestResponsesImageGenerate({
        baseUrl: req.baseUrl,
        apiKey: req.apiKey,
        toolModel: req.model,
        responsesModel: req.responsesModel,
        prompt: req.prompt,
        size: req.size,
        outputFormat: req.outputFormat,
        timeoutMs: req.timeoutMs,
        generatePath: req.generatePath,
      }),
      fallback: () => requestImage({ ...req, generatePath: '' }),
    });
  }
  return requestImage(req);
}

async function edit(request = {}) {
  const mode = request.maskImage ? 'masked-edit' : 'reference-assisted';
  const req = {
    baseUrl: request.baseUrl,
    apiKey: request.apiKey,
    model: request.model,
  ...
  if (supportsResponsesReferenceMode(req.editPath, mode)) {
    return requestWithFallback({
      fileBase: request.id || 'image',
      ...
      primary: () => requestResponsesImageEdit({
        ...
        editPath: req.editPath,
      }),
      fallback: () => requestImageEdit({ ...req, editPath: '' }),
    });
  }
  return requestImageEdit(req);
}
