...
export function PromptWorkspace({ round, planVersions, loading, onRefresh }) {
  ...
  return <section className="prompt-stage">
    ...
    {loading ? <div className="empty-stage"><RefreshCw size={26} className="spin" /><p>正在读取计划版本。</p></div> : !planVersions.length ? <div className="empty-stage"><Layers3 size={30} strokeWidth={1.15} /><p>当前轮次还没有可展示的计划版本。</p></div> : <>
      ...
      <div className="prompt-version-grid">{planVersions.map((version) => { const presentation = planPresentation(version.plan); return <article key={version.id || version.planVersion} className="prompt-version-card"><header><div><span className={'prompt-state ' + stateTone(version.state)}>{version.state === 'confirmed' && <Check size={13} />}{planStateLabel(version.state)}</span><h3>计划 v{version.planVersion}</h3></div><small>{version.confirmedAt ? '已于 ' + new Date(version.confirmedAt).toLocaleString('zh-CN') + ' 确认' : version.createdAt ? '创建于 ' + new Date(version.createdAt).toLocaleString('zh-CN') : '时间未知'}</small></header><dl><div><dt>操作</dt><dd>{presentation.operation}</dd></div><div><dt>计划数量</dt><dd>{presentation.itemCount === null ? '未设置' : presentation.itemCount + ' 张'}</dd></div><div><dt>输出规格</dt><dd>{presentation.output}</dd></div><div><dt>参考素材</dt><dd>{presentation.references.length ? presentation.references.length + ' 张已绑定' : '无'}</dd></div></dl><section className="prompt-copy"><span>提示词</span><p>{presentation.prompt}</p></section>{presentation.references.length > 0 && <div className="prompt-references"><ImagePlus size={14} /><span>已绑定参考素材，生成前会由预检验证能力。</span></div>}</article>; })}</div>
