...
`internal/workspace_state.json` 是唯一产品状态源。五个页面的 `primaryAction` 和 `replySuggestions` 都从这里派生。渲染器只读取 `internal/view_models/*.json`，不读取旧运行记录做业务判断。
...
- `internal/asset_library.json`: 用户资产库，所有用户可见资产都必须有 `userTitle` 和 `userStatus`。
...
- `assets/selected/`: 用户选中或系统建议优先看的候选。
- `assets/exports/`: 可交付成果、交付图和报告。
