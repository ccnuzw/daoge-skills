...
以下结果来自本轮工作树，不代表已发布的 `daoge-pic-5.13.0.tgz`。下一区块由 `npm run verify:evidence` 从真实构建与真实回归产出，**请勿手改**；`npm run verify:evidence:check` 会在区块与代码不一致时失败，避免证据再次与实现脱节。
...
- 生成时间：`2026-09-14T01:53:20.305Z`（Node.js `v22.22.2`，`darwin`）。
- `npm run build`：通过；Vite 转换 1624 个模块，Workbench 产物 JS 640.25 kB、CSS 218.85 kB，只有非阻断大小提示。
- 全量回归：`node --test --test-concurrency=1 tests/vnext/*.test.js` 共 483 项，`481 通过`、`0 失败`、`0 取消`、`2 条件跳过`、`0 待办`。
...
- `npm run test:package`：通过；安全临时 pack/consumer 流程 `unexpected=0`、`maps=0`、`retired=0`、`sensitive=0`，安装、真实 bin、help、register-skill、doctor 和 `sharp` 均通过。脚本使用临时 pack 根目录，不覆盖稳定制品。需要把发布清单文件数一并写入上方区块时，改用 `npm run verify:evidence -- --with-package`。
- 本轮新增/修复已实际覆盖：Studio/Profile/configVersion 隔离的跨 Worker cooldown 恢复、Review project/task 层级 fail-closed、评审反馈哈希化、Project Overview 质量聚合卡、谱系 minimap 240 节点预算、普通 brief 的文本视图隔离和加载中覆盖状态；以及本次收口引入的 Schema v32（把计划声明的成本估算冻结到 `generation_runs`，使预算闸门能累计真实已声明支出）、预检 `reference_requires_edit`（拒绝「声明了参考图或遮罩却把 operation 设为 generate」的计划）、线协议参考图一致性断言，与 `build:vnext` 的类型检查前置闸门。
- 稳定正式制品 `daoge-pic-5.13.0.tgz` SHA-256 仍为 `d2774a8d905a510b743b61f89292bc366a7b764af258dfae6fba0bd576ec552e`；本轮未覆盖或重打包该制品，未调用真实图片 Provider，也未产生计费生成请求。
