...
本节对应 `daoge-pic-v5.14.1` 的发布源码与 GitHub Release `.tgz` 制品。5.14.0 及更早章节保持历史证据，不与本节的运行时、Studio schema 或验证计数混用。本节的机器区块由 `npm run verify:evidence` 从真实构建与真实回归产出，**请勿手改**；`npm run verify:evidence:check` 会在区块与代码不一致时失败，避免证据与实现脱节。
