...
const CATEGORY_COPY = Object.freeze({
  ...
  skill_only: { code: 'skill_only_action', message: '这个操作要用到生成服务密钥，只能在本地命令行里执行。请在会话里让智能体做，或运行对应的 daoge 命令。' },
  secret_backend: { code: 'provider_secret_backend_policy', message: '本地后台服务读到的生成服务密钥存法，与工作区里已有的 Profile 不一致，因此拒绝了这次读写。这不是连接故障：请用 DAOGE_PIC_PROVIDER_SECRET_BACKEND=plaintext 重启后台服务后重试。' },
  ...
  provider: { code: 'provider_error', message: '生成服务暂时没能完成这次请求。它返回的细节已隐去隐私；请查看这张出图记录，或回到会话处理。' },
  media: { code: 'media_error', message: '图片或媒体文件未通过安全校验。请检查文件格式、大小或遮罩用途后继续。' },
  unknown_outcome: { code: 'outcome_unknown', message: '外部请求结果不明确。请先核实这张的状态或素材结果，不能自动重放。' },
...
const PRESENTATION_COPY = Object.freeze({
  ...
  secret_backend: { tone: 'warning', title: '生成服务密钥读不到', failure: true },
  conflict: { tone: 'warning', title: '状态已变化', failure: true },
  validation: { tone: 'warning', title: '需要调整后继续', failure: true },
  provider: { tone: 'danger', title: '生成服务没完成请求', failure: true },
