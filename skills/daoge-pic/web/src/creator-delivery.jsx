...
function DeliveryAssetGrid({ title, hint, assets, onDownload, onCopy, onRemove, emptyAction, onArchive, disabled = false }) {
  ...
  const chooseAll = () => {
    if (disabled) return;
    setChosen(chosen.length === assets.length ? [] : assets.map(archiveKey));
  };
  return <section className="creator-delivery-assets" aria-busy={disabled}><header><div><p className="eyebrow">{title}</p><h3>{assets.length ? assets.length + ' 张图片' : '还没有图片'}</h3><span>{hint}</span></div>{assets.length ? <div className="creator-delivery-archive-actions">{onArchive && <><button type="button" className="outline-button" disabled={disabled || !selected.length} onClick={() => onArchive(selected)}><Download size={15} />打包下载 {selected.length} 张</button><button type="button" className="icon-button" disabled={disabled} title={chosen.length === assets.length ? '取消全选打包图片' : '全选打包图片'} aria-label={chosen.length === assets.length ? '取消全选打包图片' : '全选打包图片'} onClick={chooseAll}>{chosen.length === assets.length ? <CheckSquare size={16} /> : <Square size={16} />}</button></>}{emptyAction}</div> : emptyAction}</header>{assets.length ? <div className="creator-delivery-asset-grid">{assets.map((asset) => <article key={archiveKey(asset)} className={chosenSet.has(archiveKey(asset)) ? 'is-archive-selected' : ''}><img src={asset.fileUrl || '/api/assets/' + encodeURIComponent(asset.id) + '/file'} alt="" /><label className="creator-asset-archive-select" title="加入打包下载"><input type="checkbox" disabled={disabled} checked={chosenSet.has(archiveKey(asset))} onChange={() => toggle(asset)} /><span>{chosenSet.has(archiveKey(asset)) ? <CheckSquare size={15} /> : <Square size={15} />}</span></label><div><strong>{asset.display?.label || '交付图片'}</strong><span>{asset.review?.decision === 'keep' ? '已保留' : '已完成交付'}</span></div><div className="creator-asset-actions"><button type="button" disabled={disabled} title="下载原图" aria-label="下载原图" onClick={() => onDownload(asset)}><Download size={16} /></button><button type="button" disabled={disabled} title="复制图片" aria-label="复制图片" onClick={() => void onCopy(asset)}><Copy size={16} /></button>{onRemove && <button type="button" disabled={disabled} title="移出这批图片" aria-label="移出这批图片" onClick={() => onRemove(asset)}><X size={16} /></button>}</div></article>)}</div> : <div className="creator-delivery-empty"><ImagePlus size={24} strokeWidth={1.2} /><span>从项目资产选择并保留图片后，会出现在这里。</span></div>}</section>;
}
...
function CreatorDeliveryHistory({ deliveries, assets, selectionReady, deliveryBusyId, disabled, onAction, onOpenAssets, onDownload, onCopy, onArchiveDelivery }) {
  ...
  return <section className="creator-delivery-history"><header><div><p className="eyebrow">已保存的交付</p><h2>继续处理或领取图片</h2></div><span>{deliveries.length} 批</span></header>{deliveries.map((delivery) => {
    ...
    const frozenAssets = (delivery.items || []).map((item) => {
      const activeAsset = assets.find((asset) => asset.id === item.assetId || asset.id === item.id);
      return { ...(activeAsset || { id: item.assetId, kind: item.asset?.kind, review: item.review, display: { label: '第 ' + item.sequence + ' 张成品' } }), archiveKey: item.sequence, fileUrl: deliveryFileUrl(delivery.id, item.sequence), downloadUrl: deliveryFileUrl(delivery.id, item.sequence, true) };
    });
