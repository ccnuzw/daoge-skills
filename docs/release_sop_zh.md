...
本 SOP 针对当前 vNext 发布模型：运行时版本、Skill protocol 版本和发布制品版本相互独立。当前工作树的运行时为 `6.1.0`，协议为 `daoge-pic-skill-protocol/3.1.0`，兼容范围为 `>=6.0.0 <7.0.0`。发布渠道是 GitHub Release 的不可变 `.tgz`，不是 npm registry；不要把工作树、`main` 或旧 daemon 当作正式发布物。
...
(cd skills/daoge-pic && shasum -a 256 daoge-pic-6.1.0.tgz > daoge-pic-6.1.0.tgz.sha256)
npm --prefix skills/daoge-pic run test:package
npm --prefix skills/daoge-pic run verify:evidence -- --with-package
(cd skills/daoge-pic && shasum -a 256 -c daoge-pic-6.1.0.tgz.sha256)
```
...
```bash
git tag -a daoge-pic-v6.1.0 -m "daoge-pic 6.1.0"
git push origin daoge-pic-v6.1.0
gh release create daoge-pic-v6.1.0 \
  skills/daoge-pic/daoge-pic-6.1.0.tgz#daoge-pic-6.1.0.tgz \
  skills/daoge-pic/daoge-pic-6.1.0.tgz.sha256#daoge-pic-6.1.0.tgz.sha256 \
  --repo ccnuzw/daoge-skills \
  --title "daoge-pic v6.1.0" \
  --notes-file docs/daoge_pic_6.1.0_release_notes_zh.md
```

## 6. 发布后验证

```bash
gh release view daoge-pic-v6.1.0 --repo ccnuzw/daoge-skills
