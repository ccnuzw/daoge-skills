...
> **版本状态**：`daoge-pic` 当前稳定正式版本是 [5.9.0](https://github.com/ccnuzw/daoge-skills/releases/tag/daoge-pic-v5.9.0)。
...
在项目根目录安装 `daoge-pic` 稳定正式版 `5.9.0`（固定使用 `daoge-pic-v5.9.0` Release 中的不可变制品，不会随 `main` 变化）：
...
每个 Skill 独立维护版本和发布说明。更新某个 Skill 时，应只修改其自身范围内的代码、模板、测试和 README，并运行相应验证；不要因为两个 Skill 位于同一仓库而假设它们共享运行时或发布条件。`daoge-pic` 当前稳定正式版本为 [v5.9.0](https://github.com/ccnuzw/daoge-skills/releases/tag/daoge-pic-v5.9.0)。发布验证与历次版本证据在 [vNext 验证记录](./skills/daoge-pic/docs/vnext_verification_evidence_zh.md) 中分章维护；最终制品哈希由 GitHub Release 和 sidecar 在包外记录，不属于运行时包。
