...
async function readJson(response: Response, maxBytes = MAX_RESPONSE_BYTES): Promise<Record<string, unknown>> {
  ...
  if (Number.isFinite(length) && length > maxBytes) throw new Error('Provider response exceeds the configured size limit.');
  const buffer = Buffer.from(await response.arrayBuffer());
  if (buffer.length > maxBytes) throw new Error('Provider response exceeds the configured size limit.');
...
async function download(url: string, signal: AbortSignal): Promise<{ bytes: Buffer; mediaType: string }> {
  const response = await fetch(url, { signal });
  if (!response.ok) throw errorWithStatus(response.status, 'Provider image download failed.');
  const length = Number(response.headers.get('content-length') || 0);
  if (Number.isFinite(length) && length > MAX_DOWNLOAD_BYTES) throw new Error('Provider image download exceeds the configured size limit.');
  const bytes = Buffer.from(await response.arrayBuffer());
  if (bytes.length > MAX_DOWNLOAD_BYTES) throw new Error('Provider image download exceeds the configured size limit.');
  return { bytes, mediaType: imageMediaType(response.headers.get('content-type')) };
}
...
class HttpImageProvider implements ImageProvider {
  ...
  constructor(config: ResolvedProviderConfig) { this.id = config.providerId; this.config = config; }
...
export function createImageProvider(config: ResolvedProviderConfig): ImageProvider { return new HttpImageProvider(config); }
