...
export function PromptWorkspace({ round, planVersions, loading, onRefresh }) {
  ...
  if (!round) return <section className="prompt-stage empty-stage"><Layers3 size={30} strokeWidth={1.15} /><p>先从左侧选择一个批次，再查看可追溯的计划与提示词。</p></section>;
  return <section className="prompt-stage">
    <PageHeader kicker={(ROUND_PURPOSE_LABELS[round.purpose] || round.purpose) + '批次'} title="当前计划" description="确认前先看通用提示词、逐图差异、参考素材和原始计划结构；历史版本和对比放在下面。"><button type="button" className="outline-button" onClick={onRefresh} disabled={loading}><RefreshCw size={16} className={loading ? 'spin' : ''} />刷新</button><button type="button" className="outline-button" onClick={() => void copyCurrent('shared')} disabled={!current?.plan?.prompt}><ClipboardCopy size={16} />复制通用提示词</button><button type="button" className="outline-button" onClick={() => void copyCurrent('final')} disabled={!current?.plan?.prompt}><ClipboardCopy size={16} />复制全部最终提示词</button><button type="button" className="outline-button" onClick={() => void copyCurrent('json')} disabled={!current?.plan}><ClipboardCopy size={16} />复制原始数据</button>{copyNotice && <span className="prompt-copy-notice" role="status">{copyNotice}</span>}</PageHeader>
    {loading ? <div className="empty-stage"><RefreshCw size={26} className="spin" /><p>正在读取计划版本。</p></div> : !planVersions.length ? <div className="empty-stage"><Layers3 size={30} strokeWidth={1.15} /><p>当前批次还没有可展示的计划版本。</p></div> : <>
