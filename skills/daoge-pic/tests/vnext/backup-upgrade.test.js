...
test('upgrade compatibility requires a matching path-free rollback point for changes', () => {
  ...
  try {
    ...
    assert.equal(evaluateUpgradeCompatibility({ currentRuntimeVersion: '5.13.0', targetRuntimeVersion: '5.14.2', currentSchemaVersion: 27, targetSchemaVersion: 27, supportedSchemaVersion: 27, targetProtocolVersion: '2.0.0', rollbackPoint: point }).allowed, true);
    assert.equal(evaluateUpgradeCompatibility({ currentRuntimeVersion: '5.13.0', targetRuntimeVersion: '5.14.2', currentSchemaVersion: 27, targetSchemaVersion: 28, supportedSchemaVersion: 28, targetProtocolVersion: '2.0.0', rollbackPoint: null }).issues.some((item) => item.code === 'rollback_missing'), true);
