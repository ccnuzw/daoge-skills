...
**实测结论（正例）**：
- **AX 树零无名控件**（9 视图，含画布 1173 节点 / 手册 1127 节点）；创建类对话框的下拉/文本域都靠 `<label>` 包裹拿到名字（**代码扫描报的 S14 误报经 AX 树撤销**）。
...
**本轮修复（3 处，S15–S17；S14 编号归并行会话的「任务列表同构」）**：

| # | 现象 | 严重度 | 状态 |
| --- | --- | --- | --- |
| S15 | **浏览模式下右键「查看快捷键」是死点击**：面板渲染被 `editing &&` 门控，点了没有可见反应 | P2 | ✅ 已修（面板门控交给 `shortcutsOpen`）+ 浏览模式复验 |
| S16 | 快捷键表没写「编辑模式」前提：框选 / 微调 / 分组三条只在编辑模式生效 | P2 | ✅ 已修（`shortcut-model.mjs` 三条补「编辑模式：」前缀；面板与手册同源，一处改两处生效） |
| S17 | 从右键菜单打开快捷键面板后 **Esc 关不掉**（焦点留在 body，keydown 到不了画布处理器）；面板本身也没有可访问名称 | P2 | ✅ 已修（面板挂载即接焦 + `aria-label="画布快捷键"`）+ 复验（Tab → 关闭按钮、Esc 关闭） |
