...
本 SOP 针对当前 vNext 发布模型：运行时版本、Skill protocol 版本和发布制品版本相互独立。当前工作树的运行时为 `6.2.0`，协议为 `daoge-pic-skill-protocol/3.1.0`，兼容范围为 `>=6.0.0 <7.0.0`。发布渠道是 GitHub Release 的不可变 `.tgz`，不是 npm registry；不要把工作树、`main` 或旧 daemon 当作正式发布物。
...
3. 版本元数据一致：`skills/daoge-pic/package.json`、`package-lock.json`、`protocol-version.json` 和编译产物的 runtime 常量均指向当前版本；协议单独保持 `3.1.0`（与制品版本相互独立）。
...
预览必须满足：无 source/tests、无 source map、无旧 daemon、无 SQLite/日志/runtime/Provider 文件；必需的 `dist/vnext` CLI、Workbench、协议、`SKILL.md` 的按需附录（`references/` 下具名白名单：`boundaries.md`、`startup.md`、`build-identity.md`、`flow.md`、`queue.md`、`commands.md`、`recovery.md`、`delivery.md`、`state-model.md`、`provider-keys.md`、`workbench.md`、`provider.env.example`）与文档入口均存在 —— 附录是 agent 按需读取的执行策略，缺了它们 SKILL.md 里的指针就是死链，`package-smoke` 会把缺件、未列入白名单的 references 文件与敏感路径一起拒绝。正式 package smoke 还会安装临时 consumer，验证 bin、`register-skill`、`doctor` 和 `sharp`。
...
(cd skills/daoge-pic && shasum -a 256 daoge-pic-6.2.0.tgz > daoge-pic-6.2.0.tgz.sha256)
npm --prefix skills/daoge-pic run test:package
npm --prefix skills/daoge-pic run verify:evidence -- --with-package
(cd skills/daoge-pic && shasum -a 256 -c daoge-pic-6.2.0.tgz.sha256)
```
...
```bash
git tag -a daoge-pic-v6.2.0 -m "daoge-pic 6.2.0"
git push origin daoge-pic-v6.2.0
gh release create daoge-pic-v6.2.0 \
  skills/daoge-pic/daoge-pic-6.2.0.tgz#daoge-pic-6.2.0.tgz \
  skills/daoge-pic/daoge-pic-6.2.0.tgz.sha256#daoge-pic-6.2.0.tgz.sha256 \
  --repo ccnuzw/daoge-skills \
  --title "daoge-pic v6.2.0" \
  --notes-file docs/daoge_pic_6.2.0_release_notes_zh.md
```

## 6. 发布后验证

```bash
gh release view daoge-pic-v6.2.0 --repo ccnuzw/daoge-skills
...
- 5.14.x 及更早版本的 daemon 不兼容；6.0.0 与本版本同处 `>=6.0.0 <7.0.0` 兼容区间，可平滑共存，同一工作区仍不得混用旧 `prepare`/`execute`/`ingest` 工作流。协议名/版本为 `daoge-pic-skill-protocol/3.1.0`，不要把它写成制品版本。
