...
interface StoredRun { id: string; round_id: string; status: GenerationRun['status']; provider_snapshot_json: string; plan_snapshot_json: string; requested_concurrency: number | null; version: number; plan_version: number; created_at: string; updated_at: string; }
...
function publicProviderSnapshot(value: string): Record<string, unknown> {
  ...
  const safe: Record<string, unknown> = {};
  for (const key of ['providerId', 'model']) if (typeof provider[key] === 'string') safe[key] = provider[key];
...
function run(row: StoredRun): PublicGenerationRun { const rawPlan = parseObject(row.plan_snapshot_json); return { id: row.id, roundId: row.round_id, status: row.status, providerSnapshot: publicProviderSnapshot(row.provider_snapshot_json), planSnapshot: publicRunPlanSnapshot(rawPlan), requestSummary: publicRunRequestSummary(rawPlan), requestedConcurrency: row.requested_concurrency === null || row.requested_concurrency === undefined ? null : Number(row.requested_concurrency), version: row.version, planVersion: Number(row.plan_version), createdAt: row.created_at, updatedAt: row.updated_at }; }
...
export interface PublicGenerationRun { id: string; roundId: string; status: GenerationRun['status']; providerSnapshot: Record<string, unknown>; planSnapshot: PublicRunPlanSnapshot; requestSummary: PublicRunRequestSummary; requestedConcurrency: number | null; version: number; planVersion: number; createdAt: string; updatedAt: string; }
