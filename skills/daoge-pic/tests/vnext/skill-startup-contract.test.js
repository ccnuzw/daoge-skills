...
test('5.8.0 is the stable GitHub Release contract while 5.7.0 remains immutable history', () => {
  const packageJson = JSON.parse(read('package.json'));
  const packageLock = JSON.parse(read('package-lock.json'));
  const currentDocs = `${skill}\n${readme}\n${spec}`;

  assert.equal(packageJson.version, '5.8.0');
  assert.equal(packageLock.version, '5.8.0');
  assert.equal(packageLock.packages[''].version, '5.8.0');
  for (const document of [skill, readme, spec, evidence]) {
    assert.match(document, /5\.8\.0/);
  }
  assert.match(currentDocs, /稳定正式版本[^。\n]{0,120}5\.8\.0/);
  assert.doesNotMatch(currentDocs, /5\.8\.0[^。\n]{0,80}尚未发布/);
  assert.doesNotMatch(currentDocs, /稳定正式发布(?:仍)?(?:为|是)[^。\n]*5\.7\.0/);
  assert.match(readme, /daoge-pic-v5\.8\.0\/daoge-pic-5\.8\.0\.tgz/);
  ...
  const releaseEvidence = markdownSection(evidence, '## 5. daoge-pic 5.8.0 发布验证证据');
  assert.match(releaseEvidence, /daoge-pic-v5\.8\.0[\s\S]*GitHub Release[^\n]*\.tgz/);
  assert.match(releaseEvidence, /build：PASS[\s\S]*238\/238[\s\S]*60\/60[\s\S]*96 个文件[\s\S]*sensitive=0/);
  assert.match(releaseEvidence, /BEGIN EXCLUSIVE[\s\S]*SIGKILL[\s\S]*四个并发首次 CLI[\s\S]*250ms[\s\S]*outcome_unknown/);
  assert.match(releaseEvidence, /4 个并发首次 CLI[\s\S]*1 个 `opened:true`[\s\S]*3 个 `reused:true`[\s\S]*4 个彼此隔离的 Studio Session/);
  assert.match(releaseEvidence, /a454326e3b5c47ce0f9c54629f245eaf5c6d8f438c524876a52221eddca030fd[\s\S]*superseded[\s\S]*不得作为最终/);
  assert.match(releaseEvidence, /SHA-256 不写入包内文档[\s\S]*\.tgz\.sha256[\s\S]*GitHub Release/);
  assert.match(releaseEvidence, /没有调用任何真实外部图片 Provider[\s\S]*重启 Codex[\s\S]*Skill registry/);
  assert.match(releaseEvidence, /GitHub Release 不可变 `\.tgz`[\s\S]*不表示 npm registry 已发布/);
  assert.doesNotMatch(releaseEvidence, /当前稳定正式发布仍为 5\.7\.0|5\.8\.0 仍为尚未发布候选|不宣称 GitHub Release/);
});
