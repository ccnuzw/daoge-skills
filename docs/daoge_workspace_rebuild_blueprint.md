...
- 入口页面契约包含 `references/examples/examples_catalog.html`。
- run 内用户页面契约只包含 `workspace/*.html`。
- 稳定 debug 契约包含 `debug/prompts.generated.json`，但它不是普通用户可见入口。
- 核心机器契约落盘在 `internal/`：`internal/run_plan.json`、`internal/execution_manifest.json`、`internal/issue_queue.json`、`internal/asset_library.json`、`internal/workspace_state.json`、`internal/view_models/*.json`。
- 后文历史章节中的 `workspace_state.json`、`workspace_assets.json`、`workspace_timeline.json` 是逻辑模型名；本次发布不新增顶层同名文件。
- 本次发布不冻结跨 run 总控状态文件；如未来恢复跨 run 总入口，必须另定契约。
- `workbench_state.json` 只作内部派生工作台快照，不作为主状态源。

终版生成策略：

- run 内主链页默认生成，并位于或镜像到 `workspace/`。
- 额外准备详情模块只在 `prepare-details` / `all` 生成，不影响 `workspace/prepare.html` 默认生成。
- 额外结果详情模块只在 `result-details` / `all` 生成，不影响 `workspace/results.html` 默认生成。
...
1. `references/examples/examples_catalog.html`：模板入口
2. `workspace/index.html`：单任务工作台首页
3. `workspace/prepare.html`：准备工作台
4. `workspace/results.html`：结果工作台
5. `workspace/issues.html`：问题工作台
6. `workspace/record.html`：任务档案

说明：

- 模板入口与五个工作台页共同构成“个人工作流 UI”的主外壳
- 普通用户默认只走这套主链
...
这是本次新增的核心层。发布落盘路径以 `internal/` 为准；旧文里不带目录的状态文件名只表示逻辑模型，不表示顶层稳定文件。
...
它服务的是入口层，而不是 run 内工作台。本次发布不冻结独立顶层 `entry_state.json`。
...
- 页面默认优先读取 `internal/view_models/*.json` 或上述 `internal/` 核心契约
- 其它 JSON 只作为底层数据源存在
...
- `internal/local_execution_raw.json`
- `internal/batch_plan.json`
- `debug/job_state.json`
- `debug/checkpoint.json`
- `debug/stage_plan.json`
...
- 不作为普通用户主入口
- 主要给脚本和维护者使用
- 旧版顶层恢复文件名只作历史称呼；新文档引用必须使用 `internal/` 或 `debug/` 落盘路径
...
- 除稳定 debug 契约 `debug/prompts.generated.json` 外，后续调试产物逐步归入 `debug/` 或非契约内部目录
- 对普通用户默认隐藏
- 只在维护者模式或专业模式下暴露
- 维护者可以查看 7.2 中的核心机器契约，但这些契约不归入调试资产，字段变更必须同步契约测试
...
- 旧审阅页：`review_board.html`
- 旧完成页：`completion_board.html`
- 旧运行概览：`run_overview.html`
- 旧补跑页：`rerun_board.html`
- 旧准备预览页：`preflight_board.html` / `prompt_preview.html`
- `assets_board.html`
- 已删除历史入口：`result_hub.html`、`daoge_portal.html`

处理方式：

- 准备内容进入稳定 `workspace/prepare.html`
- 结果内容进入稳定 `workspace/results.html`
- `result_hub.html` 与 `daoge_portal.html` 已从生成链路删除

## 9. 统一状态模型设计

### 9.1 `entry_state` 逻辑模型建议结构
...
```json
{
  ...
  "recommendedNextStep": {
    "label": "生成预检工作台",
    "target": "workspace/prepare.html"
  },
  "updatedAt": "2026-05-21T00:00:00.000Z"
}
```

### 9.2 `internal/workspace_state.json` 建议结构
...
```json
{
  ...
  "routes": {
    ...
    "result": "workspace/results.html",
    "exception": "workspace/issues.html",
    "record": "workspace/record.html",
...
### 9.3 `internal/asset_library.json` 建议结构
...
```json
{
  ...
  "exceptionItems": [],
  "keyFiles": {
    "manifest": "internal/execution_manifest.json",
    "jobState": "debug/job_state.json",
    "promptPreview": "workspace/prepare.html"
  }
}
```
...
本次发布不冻结独立顶层 `workspace_timeline.json`。页面需要的时间线数据由 `internal/execution_manifest.json`、`internal/workspace_state.json` 或 `internal/view_models/*.json` 派生。
...
### 10.1 `references/examples/examples_catalog.html`
...
- 本次发布不冻结跨 run 总入口页面
- 模板浏览入口继续使用 `references/examples/examples_catalog.html`
- 单任务入口统一使用 `workspace/index.html`

如未来恢复跨 run 总入口，必须使用新文件名和新状态契约，不能复用 `workspace/index.html`。

### 10.3 `workspace/index.html`
...
### 11.3 local-edit / rerun 是专业路径，不应误删
...
这些职责可以在现有 `scripts/daoge.js prepare` / `ingest` 或内部 builder 中完成，不冻结新增脚本名：

- 构建入口状态逻辑模型
- 构建 `internal/workspace_state.json`
- 构建 `internal/asset_library.json`
- 构建 `internal/execution_manifest.json` 派生摘要
...
- 模板入口：`render_example_catalog_board.js` 优先读入口状态逻辑模型
- `workspace/index.html`：`src/renderers/workspace_page.js` 读 `internal/workspace_state.json` 或 `internal/view_models/index.json`
- `workspace/prepare.html`：`src/renderers/workspace_page.js` 读 `internal/workspace_state.json + internal/run_plan.json + internal/asset_library.json`
- `workspace/results.html`：`src/renderers/workspace_page.js` 读 `internal/workspace_state.json + internal/execution_manifest.json + internal/asset_library.json`
- `workspace/issues.html`：`src/renderers/workspace_page.js` 读 `internal/workspace_state.json + internal/issue_queue.json + internal/asset_library.json`
- `workspace/record.html`：`src/renderers/workspace_page.js` 读 `internal/workspace_state.json + internal/run_plan.json + internal/execution_manifest.json`
...
1. 新增入口状态逻辑模型
2. 新增 `internal/workspace_state.json`
3. 新增 `internal/asset_library.json`
4. 新增 `internal/execution_manifest.json` 派生摘要
...
2. 改 `workspace/index`
3. 改 `workspace/prepare`
4. 改 `workspace/results`
5. 改 `workspace/issues`
6. 改 `workspace/record`
...
4. `preflight_board.html` / `prompt_preview.html` / `assets_board.html` 降级
...
说明：本节按用户默认推进、状态处理、归档查看拆分页面职责，不缩减 0 节和 6.1 定义的完整稳定主链。
...
2. `workspace/prepare.html`
3. `workspace/results.html`

### 按状态进入的主链页

1. `workspace/issues.html`

### 统一人话归档页

1. `workspace/record.html`

### 条件专业页

1. `storyboard_board.html`

### 统一入口状态

1. 入口状态逻辑模型（本次发布不冻结顶层 `entry_state.json`）

### 核心机器契约

1. `internal/run_plan.json`
2. `internal/execution_manifest.json`
3. `internal/issue_queue.json`
4. `internal/asset_library.json`
5. `internal/workspace_state.json`
6. `internal/view_models/*.json`

### 底层运行资产

1. `internal/local_execution_raw.json`
2. `internal/batch_plan.json`
3. `debug/job_state.json`
4. `debug/checkpoint.json`
5. `debug/stage_plan.json`

### 专业与调试资产

1. `debug/prompts.generated.json`
2. 其它 `debug/` 中间产物
...
1. 设计入口状态逻辑模型
2. 设计并落地 `internal/workspace_state.json`
3. 从现有入口资产和运行资产自动聚合出第一版状态
4. 先让 `workspace/index.html` 和 `examples_catalog.html` 逐步读取统一状态
