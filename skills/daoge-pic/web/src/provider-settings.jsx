...
export function ProviderSettings({ request, onDismiss, onChanged }) {
  ...
  const save = async (restart) => {
    ...
    if (mode === 'edit' && (form.baseUrlAction === 'clear' || form.apiKeyAction === 'clear') && !window.confirm('清除连接信息会让该 Profile 暂时不可用。是否继续？')) return;
  ...
  const action = async (name) => {
    if (!selected) return;
    if (name === 'delete' && !window.confirm('删除 Profile“' + selected.name + '”？此操作不会删除历史运行。')) return;
    setBusy(name); setError(''); setFeedback('');
    try {
      const suffix = name === 'copy' ? '/copy' : name === 'activate' ? '/activate' : name === 'delete' ? '/delete' : '/' + name;
      const result = await request('/api/providers/' + encodeURIComponent(selected.id) + suffix, { method: 'POST', idempotencyKey: crypto.randomUUID(), body: {} });
  ...
  return <AccessibleDialog className="provider-settings-dialog" label="Provider Profile 设置" onDismiss={onDismiss}>
