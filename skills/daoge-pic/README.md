...
当前包版本：5.2.0。它是一次不兼容的 vNext 替换，不读取或迁移旧任务 JSON、旧静态工作区、旧目录状态或旧运行记录。
...
5. 预检证据与当前安全 Provider 快照一致时，才创建持久运行；daemon 从 SQLite 队列领取运行项。
...
- daemon 在启动时固定 Provider 配置；修改 provider.env 会留下需重启的审计事件，不能影响正在运行的工作。
