...
DAOGE Interactive Image Batch 是一个面向批量生图工作流的 Codex Skill 和本地工作台。它把自然语言 brief 先整理为可检查的 `task_spec.json`，生成结构化提示词与批次计划；随后可以在本地调用图片 provider，也可以把提示词交给宿主原生工具、第三方工作台或自建脚本，最后将结果统一回填、筛选、补跑和导出。
