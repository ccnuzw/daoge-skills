...
第三批的目标不是继续扩治理，而是让 `host-native-image-tool` 路径拥有稳定、可检查、可复用的最小产物链。
...
1. 新增 `host-native` 最小产物脚本
2. 生成 Markdown / JSON / HTML 三类轻量摘要产物
3. 将该脚本接入测试与统一 smoke
4. 把新的最小产物链写回文档
...
### Task 1. 新增 host-native 最小产物脚本

新增脚本：

- `skills/interactive-image-batch/scripts/build_host_native_prompt_pack.js`
...
1. 新增本计划文档
2. 实现 `build_host_native_prompt_pack.js`
3. 补 smoke test
4. 更新 `SKILL.md`
...
1. 存在可执行的 `build_host_native_prompt_pack.js`
2. 能稳定输出 JSON / Markdown / HTML 三份摘要产物
