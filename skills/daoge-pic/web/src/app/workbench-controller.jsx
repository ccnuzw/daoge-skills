...
function ImageInspectorDialog({ assets, zoom, selectedAssetIds, selectionBusyIds, selectedProject, selectedTask, fallbackTask, selectedRound, readOnly = false, onClose, onZoom, onToggleDeliverable, onOpenDerive, onAddReference, onReject, onOpenReference }) {
  ...
  // 当前看第几张：切图靠 ←→，手不必回鼠标。
  const [focusIndex, setFocusIndex] = useState(/** @type {any} */ (0));
  const [fullscreen, setFullscreen] = useState(/** @type {any} */ (false));
...
export const ASSET_PAGE_SIZE_KEY = 'daoge-pic:asset-page-size';


export const ASSET_PREVIEW_FIT_KEY = 'daoge-pic:asset-preview-fit';


export const BASE_DOCUMENT_TITLE = document.title || 'DAOGE Pic Studio';


export const DELIVERY_COMPLETION_PREFIX = 'daoge-pic:delivery-completion:';
...
export const EMPTY_LINEAGE_ASSET_COVERAGE = /** @type {any} */ (Object.freeze({ loaded: 0, total: 0, loading: true }));


export const RAIL_COLLAPSE_KEY = 'daoge-pic:rail-collapsed';


export function deliveryArchiveUrl(deliveryId, sequences) { const params = new URLSearchParams(); for (const sequence of sequences) params.append('sequence', String(sequence)); return '/api/deliveries/' + encodeURIComponent(deliveryId) + '/archive?' + params.toString(); }



export function materialNeedCompletionCounts(materialNeeds, assets, referenceMaterials) {
...
export function materialNeedsForContext(project, task, round, templates = EMPTY) {
...
export function normalizeReferenceMaterials(plan = {}) {
  const values = [];
  const seen = new Set();
...
export function planWithReferenceMaterials(plan = /** @type {any} */ ({}), materials = []) {
...
export function projectArchiveUrl(projectId, assetIds) { const params = new URLSearchParams(); for (const assetId of assetIds) params.append('assetId', assetId); return '/api/projects/' + encodeURIComponent(projectId) + '/assets/archive?' + params.toString(); }



export function rejectFeedbackToDerivedForm(feedback, assets) {
...
export function runExecutionPresentationFromCounts(run, statusCounts) {
...
export function useWorkbenchController() {
  ...
  const [deliveryCreating, setDeliveryCreating] = useState(/** @type {any} */ (false));
  ...
  const [creationBusy, setCreationBusy] = useState(/** @type {any} */ (false));
  const [creationError, setCreationError] = useState('');
  const [referenceDialog, setReferenceDialog] = useState(null);
  const [referenceBusy, setReferenceBusy] = useState(/** @type {any} */ (false));
  ...
  const [derivedBusy, setDerivedBusy] = useState(/** @type {any} */ (false));
  const [derivedError, setDerivedError] = useState('');
  const [rejectDialog, setRejectDialog] = useState(null);
  const [rejectBusy, setRejectBusy] = useState(/** @type {any} */ (false));
  ...
  const [runtimeRepairing, setRuntimeRepairing] = useState(/** @type {any} */ (false));
  ...
  const [batchBusy, setBatchBusy] = useState(/** @type {any} */ (false));
  ...
  const [progressRevision, setProgressRevision] = useState(/** @type {any} */ (0));
  useEffect(() => {
  ...
  const [unreadCompletions, setUnreadCompletions] = useState(/** @type {any} */ (0));
...
await refresh();
  ...
  // loading 不再从 hook 里 return JSX（那会把「props 对象」和「元素」混成一个返回值，App 展开即崩）
  // —— 由 App 依据这个标志渲染加载壳（行为不变）。
