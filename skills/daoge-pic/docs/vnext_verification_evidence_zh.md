...
当前稳定正式版本为 [`6.1.0`](https://github.com/ccnuzw/daoge-skills/releases/tag/daoge-pic-v6.1.0)；`6.0.0`、`5.14.2` 及更早章节保持不可变历史证据。下列章节按版本隔离发布事实。
...
本节对应 `daoge-pic-v6.0.0` 的发布源码与 GitHub Release `.tgz` 制品。6.1.0 及更早章节保持历史证据，不与本节的运行时、Studio schema 或验证计数混用。以下为 6.0.0 冻结的机器证据（由 `npm run verify:evidence` 在发布时产出）。
...
本节对应 `daoge-pic-v6.1.0` 的发布源码与 GitHub Release `.tgz` 制品。6.0.0 及更早章节保持历史证据，不与本节的运行时、Studio schema 或验证计数混用。本节的机器区块由 `npm run verify:evidence` 从真实构建与真实回归产出，**请勿手改**；`npm run verify:evidence:check` 会在区块与代码不一致时失败。
