...
当前稳定正式版本为 [`5.14.0`](https://github.com/ccnuzw/daoge-skills/releases/tag/daoge-pic-v5.14.0)；`5.13.0` 及更早章节保持不可变历史证据。下列章节按版本隔离发布事实。
...
本节对应 `daoge-pic-v5.14.0` 的发布源码与 GitHub Release `.tgz` 制品。5.13.0 及更早章节保持历史证据，不与本节的运行时、Studio schema 或验证计数混用。下一区块由 `npm run verify:evidence` 从真实构建与真实回归产出，**请勿手改**；`npm run verify:evidence:check` 会在区块与代码不一致时失败，避免证据与实现脱节。
