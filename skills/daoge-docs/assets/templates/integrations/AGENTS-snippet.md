...
Goal 状态以 CLI 派生的 `phase` 和 `reason_code` 为准：`not_started` 表示尚未开始，`dependency_blocked` 表示前置未完成，`executing` 表示已恢复执行，`verification_failed` 表示最近验证失败，`completed` 表示检查点链完成，`stale` 或 `blocked` 表示必须停止并回到权威文档/门禁处理。CI 应运行 `ci-check`，并对未完成 Goal 使用 `goal-status --read-only --fail-on-stale`。
