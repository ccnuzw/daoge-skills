...
const EXPECTED_ARTIFACT = '6.0.0';
const EXPECTED_PROTOCOL = 'daoge-pic-skill-protocol/3.0.0';
...
test('协议版本号的四处硬编码同步改成 3.0.0', () => {
  ...
  assert.match(frontendProtocol, /WORKBENCH_PROTOCOL_VERSION = '3\.0\.0'/, '前端协议版本必须是 3.0.0');
  ...
  assert.match(readSource('scripts/package-smoke.js'), /protocolManifest\.version !== '3\.0\.0'/, 'scripts/package-smoke.js 必须把协议版本钉在 3.0.0');
  // 测试助手也硬编码了协议头，漏了它一批接口测试会以最难看的方式坏。
  assert.match(readSource('tests/vnext/local-studio-test-helper.js'), /daoge-pic-skill-protocol\/3\.0\.0/);
  // SKILL.md 的运行时兼容范围同步。
