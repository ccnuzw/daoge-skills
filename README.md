...
> **版本状态**：`daoge-pic` 当前稳定正式版本为 [5.14.2](https://github.com/ccnuzw/daoge-skills/releases/tag/daoge-pic-v5.14.2)，发布说明见 [`5.14.2 发布说明`](./docs/daoge_pic_5.14.2_release_notes_zh.md)；`5.14.1` 及更早版本保持为不可变历史发布。
...
当前稳定版为 `5.14.2`。项目级安装使用 GitHub Release 的不可变制品和内置注册命令：
...
每个 Skill 独立维护版本和发布说明。更新某个 Skill 时，应只修改其自身范围内的代码、模板、测试和 README，并运行相应验证；不要因为两个 Skill 位于同一仓库而假设它们共享运行时或发布条件。`daoge-pic` 当前稳定正式版本为 [v5.14.2](https://github.com/ccnuzw/daoge-skills/releases/tag/daoge-pic-v5.14.2)。发布验证与历次版本证据在 [vNext 验证记录](./skills/daoge-pic/docs/vnext_verification_evidence_zh.md) 中分章记录。
