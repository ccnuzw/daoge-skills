...
test('P2 searches safe projections, compares explicit task rounds, and keeps delivery batch versions immutable', async () => {
  ...
  try {
    ...
    const project = await json(started.url, '/api/projects', { method: 'POST', key: 'p2-project', body: { name: 'P2 运营项目' } });
    const projectId = project.body.data.value.id;
    const task = await json(started.url, '/api/tasks', { method: 'POST', key: 'p2-task', body: { projectId, name: 'P2 安全检索任务', intent: { prompt: 'must-not-return-search-source', apiKey: 'must-not-return-secret', endpoint: 'https://private.example.test' } } });
    const taskId = task.body.data.value.id;
    const roundA = await json(started.url, '/api/rounds', { method: 'POST', key: 'p2-round-a', body: { taskId, purpose: 'exploration', plan: { operation: 'generate', itemCount: 2, prompt: 'must-not-return-plan-source' } } });
    const roundB = await json(started.url, '/api/rounds', { method: 'POST', key: 'p2-round-b', body: { taskId, purpose: 'variation', parentRoundId: roundA.body.data.value.id, plan: { operation: 'edit', itemCount: 1, prompt: 'must-not-return-plan-source' } } });
    const search = await json(started.url, '/api/search?q=' + encodeURIComponent('安全检索') + '&limit=1');
    ...
    const overview = await json(started.url, '/api/tasks/' + taskId + '/studio-overview?round=' + roundA.body.data.value.id + '&round=' + roundB.body.data.value.id);
    ...
    const otherTask = await json(started.url, '/api/tasks', { method: 'POST', key: 'p2-other-task', body: { projectId, name: '其他任务' } });
    const otherRound = await json(started.url, '/api/rounds', { method: 'POST', key: 'p2-other-round', body: { taskId: otherTask.body.data.value.id, purpose: 'exploration' } });
    const invalidComparison = await json(started.url, '/api/tasks/' + taskId + '/studio-overview?round=' + otherRound.body.data.value.id);
    assert.equal(invalidComparison.status, 400);

    const upload = await fetch(started.url + '/api/assets/import', { method: 'POST', headers: { 'content-type': 'image/png', 'idempotency-key': 'p2-upload', 'x-daoge-target-type': 'project', 'x-daoge-target-id': projectId }, body: png });
    const asset = (await upload.json()).data;
    await json(started.url, '/api/assets/' + asset.id + '/review', { method: 'POST', key: 'p2-keep', body: { decision: 'keep' } });
    const delivery = await json(started.url, '/api/deliveries', { method: 'POST', key: 'p2-delivery', body: { projectId, name: 'P2 已准备交付', assetIds: [asset.id] } });
    await json(started.url, '/api/deliveries/' + delivery.body.data.id + '/ready', { method: 'POST', key: 'p2-delivery-ready', body: {} });
    const batch = await json(started.url, '/api/delivery-batches', { method: 'POST', key: 'p2-batch-create', body: { projectId, name: 'P2 对外交付', deliveryIds: [delivery.body.data.id] } });
    assert.equal(batch.status, 200);
    const v1 = batch.body.data.versions[0];
    const frozenMembers = JSON.stringify(v1.members);
    assert.equal(v1.status, 'draft');
    const batchReady = await json(started.url, '/api/delivery-batch-versions/' + v1.id + '/ready', { method: 'POST', key: 'p2-batch-v1-ready', body: {} });
    assert.equal(batchReady.body.data.versions[0].status, 'ready');
    const revision = await json(started.url, '/api/delivery-batches/' + batch.body.data.id + '/revisions', { method: 'POST', key: 'p2-batch-revise', body: { deliveryIds: [delivery.body.data.id] } });
    const v2 = revision.body.data.versions[0];
    const detail = await json(started.url, '/api/delivery-batches/' + batch.body.data.id);
    const provenance = await json(started.url, '/api/assets/' + asset.id + '/provenance');
    ...
    const secondReady = await json(started.url, '/api/delivery-batch-versions/' + v1.id + '/ready', { method: 'POST', key: 'p2-batch-v1-ready-again', body: {} });
