...
test('runtime exposes protocol version separately from artifact version and rejects incompatible writers', async () => {
  ...
  try {
    ...
    assert.deepEqual(studio.body.data.protocol, { name: 'daoge-pic-skill-protocol', version: '2.0.0', runtimeVersion: '5.14.2', supportedRange: '>=2.0.0 <3.0.0' });
    ...
    assert.equal(declaration.runtimeCompatibility, '>=5.14.2 <6.0.0');
  } finally {
