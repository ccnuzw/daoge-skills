...
当前稳定正式版本为 [`6.0.0`](https://github.com/ccnuzw/daoge-skills/releases/tag/daoge-pic-v6.0.0)；`5.14.2` 及更早章节保持不可变历史证据。下列章节按版本隔离发布事实。
...
本节对应 `daoge-pic-v6.0.0` 的发布源码与 GitHub Release `.tgz` 制品。5.14.2 及更早章节保持历史证据，不与本节的运行时、Studio schema 或验证计数混用。本节的机器区块由 `npm run verify:evidence` 从真实构建与真实回归产出，**请勿手改**；`npm run verify:evidence:check` 会在区块与代码不一致时失败，避免证据与实现脱节。
...
- 生成时间：`2026-09-20T09:11:28.913Z`（Node.js `v22.23.0`，`darwin`）。
- `npm run build`：通过；Vite 转换 1703 个模块，Workbench 产物 JS 708.48 kB、CSS 233.03 kB，只有非阻断大小提示。
- 全量回归：`node --test --test-concurrency=1 tests/vnext/*.test.js` 共 918 项，`916 通过`、`0 失败`、`0 取消`、`2 条件跳过`、`0 待办`。
- `npm run test:package`：通过；发布清单 188 个文件，`unexpected=0`、`maps=0`、`retired=0`、`sensitive=0`；临时安装、真实 bin、help、register-skill、doctor 与 `sharp` 均通过。
