...
test('README leads a new user to the first run command and workbench entry', () => {
  ...
  assert.match(readme, /node scripts\/daoge\.js execute\s*(?:\\\s*)?--output-dir out\s+--env-file \.env/);
  assert.match(readme, /node scripts\/daoge\.js ingest\s*(?:\\\s*)?--results-file host_native_results\.json\s+--output-dir out/);
