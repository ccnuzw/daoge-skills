...
import { Bookmark, Info, Check, CheckSquare, Copy, Download, ImagePlus, PackageCheck, Pencil, RotateCcw, Square, X } from 'lucide-react';
...
function CreatorDeliveryFlow({ project, selection, deliveryName, deliveryCreating, completion, frozen, onDeliveryNameChange, includeCreativeRecord, onIncludeCreativeRecordChange, onCreate, onOpenAssets }) {
  ...
  const recordChecked = phase.frozen ? completion.includeCreativeRecord !== false : includeCreativeRecord;
  // 批 D（第 8 批）§7.5：步骤条**仅在有草稿时展开**——没草稿时它只是一行摘要，不占版面。
  const stepsOpen = selectedCount > 0 || deliveryCreating || frozen;
  // 2026-09-20（刀哥）：交付页不自报家门——「美女写真 / 交付图片 / 按草稿、准备、导出三个阶段…」三行整个删掉。
  // 三个阶段由下面的步骤条自己说（`data-block="delivery-steps"`，无草稿时是一行摘要 + 去选图）。
  return <section className="creator-delivery-flow" aria-busy={deliveryCreating}>{stepsOpen ? <ol data-block="delivery-steps"><li className={selectedCount ? 'is-done' : 'is-current'}><span>1</span><div><b>挑选图片</b><small>{selectedCount ? selectedCount + ' 张' : '未选择'}</small></div><button type="button" className="outline-button" disabled={locked} onClick={onOpenAssets}><Bookmark size={15} />去选图</button></li><li className={phase.step === 1 || phase.step === 2 ? 'is-current' : phase.step > 2 ? 'is-done' : ''}><span>2</span><div><b>准备文件</b><small>{phase.step === 1 ? '创建唯一草稿' : phase.step === 2 ? '准备交付' : '已准备'}</small></div></li><li className={phase.step >= 3 ? 'is-current' : ''}><span>3</span><div><b>下载</b><small>{phase.complete ? '可领取' : '等待文件'}</small></div></li></ol> : <p className="creator-delivery-idle" data-block="delivery-idle"><Info size={15} aria-hidden="true" />还没有草稿：先在创作平台或资产里挑图，再回来命名导出。<button type="button" className="outline-button" disabled={locked} onClick={onOpenAssets}><Bookmark size={15} />去选图</button></p>}<section className={'creator-delivery-create ' + (ready ? 'is-ready' : '')}><div><h3>{phase.frozen ? completion.name : status.title}</h3><p>{phase.frozen ? selectedCount + ' 张图片已锁定。失败后可继续当前步骤。' : status.body}</p></div>{ready ? <div className="creator-delivery-create-controls"><input aria-label="这批成品名称" value={effectiveName || ''} disabled={locked} onChange={(event) => onDeliveryNameChange(event.target.value)} placeholder="例如：首轮人物成品" /><label className="creator-delivery-record-option" title="一并导出 creative-record.json，记录每张图的来源批次、提示词与评审"><input type="checkbox" checked={recordChecked} disabled={locked} onChange={(event) => onIncludeCreativeRecordChange(event.target.checked)} />同时导出创作记录</label><button type="button" className="command-button" disabled={deliveryCreating} onClick={onCreate}><PackageCheck size={16} />{phase.action}</button></div> : <button type="button" className="outline-button" disabled={locked} onClick={onOpenAssets}><Pencil size={15} />去选图片</button>}</section></section>;
}
