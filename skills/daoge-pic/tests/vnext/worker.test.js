...
test('worker archives generated bytes and records a managed Studio asset', async () => {
  ...
  try {
    ...
    const asset = fixture.db.prepare('SELECT id, storage_path, source_json, content_hash, project_id FROM assets').get();
    ...
    // 归属升格为列（方案 7.7.1 / 7.2）：产出图落库即写死项目，槽位直接带 asset_id。
    assert.equal(asset.project_id, fixture.projectId, 'generated asset must record its project at persist time');
    const slot = fixture.db.prepare('SELECT id, asset_id FROM run_items WHERE run_id = ?').get(fixture.run.value.id);
    assert.equal(slot.asset_id, asset.id, 'run item must point at its produced asset without parsing result_json');
  } finally {
    cleanup(fixture);
  }
});

test('generated asset persister detects Provider bytes when declared media type is wrong', async () => {
  const fixture = setupRun();
  const jpeg = await sharp({ create: { width: 1, height: 1, channels: 3, background: '#ffffff' } }).jpeg().toBuffer();
  try {
    const worker = new GenerationWorker({
      db: fixture.db,
      workerId: 'worker-archive-detect-media',
      providerConfig: fixture.config,
      provider: fakeProvider(async () => ({ bytes: jpeg, mediaType: 'image/png', externalRequestId: 'remote-jpeg' }), () => ({ kind: 'unknown_outcome', code: 'unexpected', message: 'unexpected' })),
      assetPersister: new StudioGeneratedAssetPersister({ db: fixture.db, paths: fixture.initialized.paths, studioId: fixture.initialized.manifest.studioId }),
      now: () => new Date('2026-01-01T00:00:00.000Z')
    });
    assert.deepEqual(await worker.processOnce(), { claimed: 1, succeeded: 1, retrying: 0, blocked: 0, unknown: 0, cancelled: 0 });
    const asset = fixture.db.prepare('SELECT storage_path, media_type FROM assets').get();
    assert.equal(asset.media_type, 'image/jpeg');
    assert.equal(asset.storage_path.endsWith('.jpg'), true);
  } finally {
    cleanup(fixture);
  }
});
