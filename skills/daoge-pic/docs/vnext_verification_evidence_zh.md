...
本节对应 `daoge-pic-v6.1.1` 的发布源码与 GitHub Release `.tgz` 制品。6.1.0 及更早章节保持历史证据，不与本节的运行时、Studio schema 或验证计数混用。本节的机器区块由 `npm run verify:evidence` 从真实构建与真实回归产出，**请勿手改**；`npm run verify:evidence:check` 会在区块与代码不一致时失败。
...
- 生成时间：`2026-09-21T04:45:55.418Z`（Node.js `v22.23.0`，`darwin`）。
- `npm run build`：通过；Vite 转换 1705 个模块，Workbench 产物 JS 428.94 kB、CSS 233.03 kB、按需块 26 个 / 288.38 kB，只有非阻断大小提示。
- 全量回归：`node --test --test-concurrency=1 tests/vnext/*.test.js` 共 977 项，`975 通过`、`0 失败`、`0 取消`、`2 条件跳过`、`0 待办`。
